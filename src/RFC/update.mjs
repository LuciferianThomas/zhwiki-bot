  ...
  try {
    ...
    for ( const rfcQ of rfcSections ) {
      ...
      if ( Math.abs( moment().diff( moment( lastComment.timestamp ), 'days' ) ) > 30 ) {
        editPage( bot, page, ( { content: old_content } ) => {
          return { 
            ...
            summary: `[[User:LuciferianBot/task/5|機械人]]：移除不活躍討論的RFC模板`
          }
      ...
      if ( !hasId ) {
        ...
        await editPage( bot, page, ( { content: oldContent } ) => {        
          ...
          return { 
            ...
            summary: `[[User:LuciferianBot/task/5|機械人]]：更新RFC模板`
          }
...
const editLists = async ( bot, rfcs ) => {
  ...
  for ( const rfcList of Object.keys( rfcLists ) ) {
    ...
    for ( const rfc of relatedRfcs ) {
      rfcListWt += `[[${ rfc.page }#rfc_${ rfc.id }|${ rfc.page } § <strong style="font-size:115%;">${ rfc.section.replace( /\{\{/g, '{<nowiki/>{' ) }</strong>]]\<div class="cquote mw-notalk" style="padding-left:30px;">\n${ rfc.lede }</div>\n`
    }
    ...
    await editPage( bot, rfcListPage, () => {
      return {
        text: rfcListWt,
        summary: `[[User:LuciferianBot/task/5|機械人]]：更新RFC列表頁（${ relatedRfcs.length }個活躍討論）`
      }
